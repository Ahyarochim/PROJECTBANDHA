...
# UDP Video Stream Config (UNICAST ONLY - NO BROADCAST)
UDP_VIDEO_PORT = 6000

# ZMQ Config (untuk terima command dari Android, termasuk IP address)
ZMQ_COMMAND_PORT = 6000  # HARUS SAMA dengan Android app!

# Android IP (WAJIB untuk streaming)
android_ip = None  # None = belum terima, TIDAK akan kirim frame
...
# worker/send tuning
SEND_EVERY_N_FRAMES = 1        # Kirim setiap frame yang ter-capture (max throughput)
SENDER_QUEUE_MAX = 2           # small queue for frames to send (drop when full)
YOLO_EVERY_N_FRAMES = 6        # Run YOLO setiap 6 frame (kurangi CPU load)
YOLO_CONF_THRESHOLD = 0.45     # Sedikit tingkatkan untuk proses lebih cepat
...
# UDP SOCKET (untuk kirim video - UNICAST ONLY)
sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
# TIDAK pakai broadcast - hanya unicast ke Android IP
...
def UndistortFrame():
    ...
    skipped_no_ip_counter = 0  # Frame yang di-skip karena belum ada IP
    ...
    def sender_worker():
        ...
        while not sender_stop.is_set():
            ...
            try:
                ...
                # HANYA kirim jika IP Android sudah tersedia
                with android_ip_lock:
                    target_ip = android_ip
                
                if target_ip is None:
                    # Belum ada IP - skip frame ini
                    nonlocal skipped_no_ip_counter
                    skipped_no_ip_counter += 1
                    if skipped_no_ip_counter % 50 == 0:
                        print(f"[INFO] ⏳ Waiting for Android IP... (skipped {skipped_no_ip_counter} frames)")
                    continue
                ...
                try:
                    ...
                    # Kirim UNICAST ke Android IP
                    sock.sendto(header + data, (target_ip, UDP_VIDEO_PORT))
                    sent_counter += 1
                except Exception as e:
                    # log every error
                    if DEBUG:
                        print(f"[WARN] unicast send failed to {target_ip}: {e}")
            except Exception as e:
                if DEBUG:
    ...
    try:
        while True:
            ...
            # Skip brightness adjustment untuk performa (uncomment jika perlu)
            Brigthness = 1.24
            ...
            if now - last_log_time >= FPS_LOG_INTERVAL:
                ...
                with android_ip_lock:
                    target_mode = f"UNICAST ({android_ip})" if android_ip else "⏳ WAITING FOR IP"
                
                print(f"[METRICS] "
                      f"FPS={current_fps:.1f} | "
                      f"sent={sent_counter} | "
                      f"skipped={skipped_no_ip_counter} | "
        ...
        if DEBUG:
            print(f"[INFO] sent={sent_counter}, skipped={skipped_no_ip_counter}, dropped={dropped_send_counter}")
            with android_ip_lock:
                if android_ip:
                    print(f"[INFO] Final target: UNICAST to {android_ip}")
                else:
                    print(f"[WARN] ⚠️  No Android IP received - NO frames were sent!")
