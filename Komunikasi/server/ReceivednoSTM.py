...
class ZMQRobotServer:
    ...
    def send_to_stm(self):
        ...
        if not self.serial_connected or not self.ser:
            # Jika tidak terkoneksi, hanya tampilkan data (untuk testing)
            print("[SIMULATED] Would send to STM32:", end=" ")
    ...
    def parse_android_message(self, message):
        ...
        6. "ROTATE:value" - Rotate (ROT,value) [TAMBAHAN]
        7. "100.5" - Motor1 value only
        """
        try:
    ...
    def start(self, test_mode=False):
        ...
        Args:
            test_mode: If True, skip serial connection and just display received data
        """
        self.running = True
        ...
        if not test_mode:
            ...
            print("[TEST MODE] Running without STM32 connection - will only display received data")
            self.serial_connected = False
...
if __name__ == "__main__":
    ...
    # TESTING MODE: Set True untuk test tanpa STM32
    TEST_MODE = True  # <-- Ubah ke False jika mau pakai STM32
    
    print("=" * 60)
    if TEST_MODE:
        print("🧪 RUNNING IN TEST MODE (No STM32 connection)")
        print("   All received data will be displayed on screen")
    else:
        print("🤖 RUNNING IN NORMAL MODE (With STM32)")
    print("=" * 60)
    ...
    # Start server dengan test mode
    server.start(test_mode=TEST_MODE)